...
def net_edge_bps(
    funding_recv_bps: float, fees_bps: float, borrow_bps: float, slip_bps: float, fx_bps: float
) -> float:
...
def scan_opportunities(quotes: Dict, funding: Dict, cfg: Dict) -> List[Dict]:
    ...
    for sym, info in funding.items():
        edge = net_edge_bps(
            info["funding_bps"], fees_bps=6, borrow_bps=0, slip_bps=slip, fx_bps=0
        )
        if edge >= theta:
            out.append({"symbol": sym, "edge_bps": edge})
    return out
