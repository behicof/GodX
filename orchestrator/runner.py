...
CFG = {
  "thresholds": {"funding_arb":{"min_net_edge_bps":300,"max_leg_latency_ms":800,"max_slippage_bps":35}},
  "quotes": {}
}
...
def mock_market():
    ...
    funding_bps=random.choice([150,220,280,320,380,420])  # 1.5%…4.2%/8h (مثال)
...
def loop():
    ...
    while True:
        try:
            ...
            quotes_c.inc(); lag_g.set(100)
            push_metrics(r, grouping_key={"script":"runner"})

            opps = scan_opportunities(CFG["quotes"], funding, CFG)
            for op in opps:
                res = execute(op, exe, CFG)
                # (در حالت واقعی: لاگ معامله/PNL)
        except Exception:
            err_c.inc(); push_metrics(r, grouping_key={"script":"runner","err":"1"})
        time.sleep(2)
